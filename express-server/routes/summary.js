...
import { getDetailedSummary, makePDF } from "../utils/detailedSummary";
...
router.post("/abstractive/pdf", isAuthenticated, async (req, res) => {
  const text = req.body.text;
  ...
  try {
    const caseData = await getDetailedSummary(text, summary);
    const casePDF = makePDF(caseData);
